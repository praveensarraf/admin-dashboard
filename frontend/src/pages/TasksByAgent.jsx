  ...
  return (
    <div className="p-6 space-y-6 text-white">
      ...
      {tasksByAgent.length === 0 ? (
        ...
        tasksByAgent.map((group) => (
          <div key={group.agent._id} className="border border-zinc-700 rounded-lg shadow bg-zinc-800">
            <div className="p-4 border-b border-zinc-700 bg-zinc-900 rounded-t-lg">
              <h3 className="font-semibold text-lg">
                {group.agent.name} &mdash; <span className="text-sm text-muted-foreground">{group.agent.email}</span>
              </h3>
            </div>
